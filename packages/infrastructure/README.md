...
Copy [.env.example](.env.example) to `.env.local` and supply its values before deploying. Access applications and reusable policies belong to the sibling `cloudflare-infra` Terraform repository; configure this stack with its issuer and both **staging** audience outputs (`auctionoton_access_audiences` and `auctionoton_access_google_audiences`). Set the latter as `CLOUDFLARE_ACCESS_GOOGLE_AUD`. See [the coordinated rollout](../../docs/cloudflare-access.md#deployment-and-validation). Local commands can read TSM credentials from `packages/infrastructure/.env.local`, Access configuration from `apps/website/.env.local`, and an unexpired Wrangler OAuth grant from `~/.wrangler/config/default.toml`. Run `pnpm --filter @auctionoton/infrastructure exec wrangler whoami` to refresh/check that login. Environment values take precedence. The runner copies only the listed keys and never logs credential values. Workers Builds must supply its own API token; it does not read a developer's login.
...
The extension batches visible item and reagent lookups for 20 ms into `GET /items/ah/:auctionHouseId/:version?region=eu&ids=1,2`. Each request accepts up to 50 positive item IDs and returns `{ items, missingItemIds }`; an absent item remains visible as unavailable. The existing single-item route remains supported and now returns an uncached 404 for a missing price. Deploy the API before distributing the rebuilt extension, which requires the batch route. Persistent item cache keys now include region, version, auction house, and item ID, with a one-hour expiry; the old unscoped cache is removed on background startup.

Alchemy enables Native Workers Cache for successful public item and realm responses, with a five-minute shared lifetime and a one-minute browser lifetime. All other routes default to `no-store`. Published markets invalidate their `market:<version>-<region>-<house>` tag through a retryable Workflow step calling the authenticated default API entrypoint; this also invalidates cached partial batches. Existing browser and extension caches expire on their own schedules. Query strings are part of the cache key, and purge tags are scoped to the entrypoint. [Cache keys](https://developers.cloudflare.com/workers/cache/cache-keys/), [purge scope](https://developers.cloudflare.com/workers/cache/purge/).

Local workerd tests verify response headers, market tags, purge failures, and Workflow retries. Native Workers Cache is unavailable locally, so after deployment request the same populated item and realm URL twice and inspect `Cf-Cache-Status` for a shared hit. After a new market publishes, verify that the same item URL returns its updated timestamp; unrelated market tags should remain warm.

Deployed the optimizations to staging on 14 September 2026 at 18:47 UTC. Live checks confirmed `MISS → HIT` for EU and US item batches and the realm catalog. An authenticated EU market purge returned `{ purged: true }`; its next batch request was `MISS`, then `HIT`, while the US batch and realm catalog stayed `HIT`. Missing items, invalid queries, and unauthenticated private routes returned `BYPASS` with `no-store`. The website homepage/assets, real Linen Cloth history page, exact ID search, and prefix search were checked against the deployed D1-backed website. Alchemy's follow-up plan reported no changes. This verification purged the EU cache without importing a new snapshot or changing stored prices; publication-to-purge sequencing is covered by the local Workflow regression.

Run `pnpm --filter @auctionoton/extension test:browser` with the `agent-browser` CLI and Chromium installed for the rendering regression. It uses local API/storage fixtures with the real React components, QueryClient, request batcher, and HTTP client to check a 30-row batch, stable sorting portals, host list replacement, reused rows, missing reagents, and unmount cleanup.
